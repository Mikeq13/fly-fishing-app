...
const INSECT_TAXON = {
  'midge|Larva':                   'Chironomidae',
  'midge|Pupa':                    'Chironomidae',
  'midge|Adult / Emerger':         'Chironomus plumosus',
  'mayfly|Nymph':                  'Baetidae',
  'mayfly|Emerger / Cripple':      'Baetis',
  'mayfly|Adult Dun':              'Baetis rhodani',
  'mayfly|Spinner':                'Baetis',
  'caddis|Larva':                  'Trichoptera',
  'caddis|Pupa':                   'Trichoptera',
  'caddis|Adult':                  'Trichoptera',
  'caddis|Adult (Egg-Laying)':     'Trichoptera',
  'stonefly|Nymph':                'Plecoptera',
  'stonefly|Nymph (2–3 Year)':     'Pteronarcys californica',
  'stonefly|Nymph (Pre-Emergence)':'Pteronarcys californica',
  'stonefly|Adult':                'Pteronarcys californica',
  'stonefly|Adult (Egg-Laying)':   'Pteronarcys californica',
  'terrestrial|Terrestrial (on water)': 'Melanoplus',
  'streamer|Large Attractor / Baitfish':'Cottus',
  'streamer|Leech':                'Hirudinea',
  'streamer|Sculpin / Bottom Baitfish': 'Cottus',
  'stillwater|Nymph':              'Callibaetis',
};
...
function FlyImage({ taxon, flyQuery, type, label, size = 120 }) {
  ...
  useEffect(() => {
    ...
    if (taxon) {
      // iNaturalist Taxa API — returns real species photos, always images never PDFs
      fetch(`https://api.inaturalist.org/v1/taxa?q=${encodeURIComponent(taxon)}&per_page=1&photos=true`)
        .then(r => r.json())
        .then(d => {
          const url = d.results?.[0]?.default_photo?.medium_url;
      ...
      // Wikimedia Commons — bitmap filter excludes PDFs/SVGs
      const enc = encodeURIComponent(flyQuery);
      fetch(`https://commons.wikimedia.org/w/api.php?action=query&list=search&srsearch=${enc}&srnamespace=6&srlimit=5&srfiletype=bitmap&format=json&origin=*`)
        .then(r => r.json())
        .then(data => {
          const title = data?.query?.search?.[0]?.title;
          if (!title) throw new Error('no result');
          return fetch(`https://commons.wikimedia.org/w/api.php?action=query&titles=${encodeURIComponent(title)}&prop=imageinfo&iiprop=url&iiurlwidth=${size * 2}&format=json&origin=*`);
        })
        .then(r => r.json())
        .then(data => {
                              ...
                              return (
                                <div key={p} style={{ textAlign:'center', opacity: isCurrent ? 1 : 0.75 }}>
                                  <div style={{ position:'relative' }}>
                                    <FlyImage flyQuery={flyQ} type={null} label="" size={56} />
                                    {isCurrent && (
